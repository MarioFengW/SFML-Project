...
LinearDataStructureMenu::LinearDataStructureMenu(RenderWindow& window, Font& font)
    : window(window), font(font), currentSubMenu(LINEARMENU), currentOperation(NONE), showingDisplay(false) {
    initializeTexts();
}
    ...
    switch (currentSubMenu) {
        case LINEARMENU:
            promptText.setString(
                ...
                "2. Stack\n"
                "3. Queue\n\n"
                "Press number to select, B to go back"
            );
            break;
        case LINKED_LIST:
            promptText.setString(

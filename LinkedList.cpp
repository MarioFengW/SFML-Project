...
void LinkedList::display(RenderWindow &window, Font &font)
{
    ...
    int width = 50;
    int height = 30;
    int totalWidth = nodeCount * spacing;
    ...
    while (temp != nullptr)
    {
        ...
        rectangle.setFillColor(Color(173, 216, 230)); // Light blue fill color
        ...
        if (temp->next != nullptr)
        {
            Vertex line[] = {
                ...
                Vertex(Vector2f(x + spacing, y + height / 2), Color::Red)};
            window.draw(line, 2, Lines);

            // Draw the arrowhead
            Vertex arrowhead[] = {
                Vertex(Vector2f(x + spacing - 10, y + height / 2 - 5), Color::Red),
                Vertex(Vector2f(x + spacing, y + height / 2), Color::Red),
                Vertex(Vector2f(x + spacing - 10, y + height / 2 + 5), Color::Red)};
    ...
    if (head != nullptr)
    {
        ...
        headText.setPosition((windowWidth - totalWidth) / 2, y + height + 10);
        window.draw(headText);

        Vertex headLine[] = {
            Vertex(Vector2f((windowWidth - totalWidth) / 2 + width / 2, y + height + 10), Color::Red),
            Vertex(Vector2f((windowWidth - totalWidth) / 2 + width / 2, y + height), Color::Red)};
        window.draw(headLine, 2, Lines);

        Vertex headArrowhead[] = {
            Vertex(Vector2f((windowWidth - totalWidth) / 2 + width / 2 - 5, y + height + 5), Color::Red),
            Vertex(Vector2f((windowWidth - totalWidth) / 2 + width / 2, y + height), Color::Red),
            Vertex(Vector2f((windowWidth - totalWidth) / 2 + width / 2 + 5, y + height + 5), Color::Red)};
    ...
    complexityText.setPosition((windowWidth - complexityText.getLocalBounds().width) / 2, y + height + 50);

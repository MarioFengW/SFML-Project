...
SortingMenu::SortingMenu(RenderWindow &window, Font &font)
...
void SortingMenu::updatePrompt()
{
    ...
    case WAITING_FOR_ELEMENTS:
        promptText.setString("\nEnter the elements separated by spaces:\nPress Enter to start sorting, B to go back");
        break;
            ...
            if (event.key.code == Keyboard::Enter)
            {
                if (currentState == WAITING_FOR_COUNT)
                {
                    try
                    {
                        ...
                        {
                            algorithmText.setString("Please enter a number between 1 and 100");
                        }
                    }
                    catch (...)
                    {
                        algorithmText.setString("Please enter a valid number");
                    }
                }
                else if (currentState == WAITING_FOR_ELEMENTS)
                {
                    ...
                    {
                        algorithmText.setString("Please enter exactly " + to_string(numberOfElements) + " numbers");
                    }
                }
            }
        }
...
void SortingMenu::handleSortingAlgorithm()
{
    ...
    if (!sorting)
    {
        ...
        algorithmText.setString("Sorting Complete! Press B to return");
    }
}

// Function to draw the current menu and sorting visualization
